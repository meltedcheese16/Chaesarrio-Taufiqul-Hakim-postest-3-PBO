...
public class PanenManager {
    ...
    // Polymorphism: Overriding biasa
    public void tambahHasil(Sayuran s) {
        daftarHasil.add(s);
        System.out.println(">>> Komoditas berhasil ditambahkan!");
    }

    // Polymorphism: Overloading (versi tambahan)
    public void tambahHasil(String nama, int berat, String tanggal, String grade, String lokasi, String tambahan, boolean isUmbi) {
        Sayuran s;
        if (isUmbi) {
            s = new Umbi(nama, berat, tanggal, grade, lokasi, tambahan);
        } else {
            s = new Daun(nama, berat, tanggal, grade, lokasi, tambahan);
        }
        daftarHasil.add(s);
        System.out.println(">>> Komoditas berhasil ditambahkan (via overloading)!");
    }
